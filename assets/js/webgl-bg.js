/* ============================================
   WebGL Background - CPPN Shader
   White/Green animated background
   ============================================ */

(function(){
    const canvas = document.getElementById('webgl-box-bg');
    if (!canvas) return;
...
void main(){
    // Use aspect ratio correction to avoid stretching
    vec2 uv = gl_FragCoord.xy / uResolution.xy * 2.0 - 1.0;
    float aspect = uResolution.x / uResolution.y;
    uv.x *= aspect;
    uv.y *= -1.0;
    
    /* warp - more visible animation */
    uv += uWarp * vec2(
        sin(uv.y*6.283 + uTime*2.0),
        cos(uv.x*6.283 + uTime*2.0)
    ) * 0.08;

    /* CPPN - increased animation speed */
    vec4 col = cppn_fn(uv,
        0.15*sin(0.8*uTime),
        0.15*sin(1.2*uTime),
        0.15*sin(0.9*uTime)
    ...
    /* ── colour remap via HSL ── */
    vec3 hsl = rgb2hsl(col.rgb);
    hsl.z = 1.0 - hsl.z;          /* invert luminance : noir → blanc */
    hsl.x = 0.33;                 /* force teinte = vert */
    hsl.y = min(hsl.y * 1.5, 1.0); /* increase saturation for more green */
    col.rgb = hsl2rgb(hsl);

    /* scanlines */
    float sv = sin(gl_FragCoord.y * uScanFreq)*0.5 + 0.5;
    col.rgb *= 1.0 - (sv*sv)*uScan;
    ...
    /* params - increased speed for more visible animation */
    const P = {
        hueShift:  155,
        noise:     0.03,
        scan:      0,
    ...
    gl.uniform1f(uScan,     P.scan);
    gl.uniform1f(uScanFreq, P.scanFreq);
    ...
    function loop(){
        ...
        // Check resize on each frame (throttled by the check inside resize())
        resize();
        
        gl.uniform1f(uTime, ((performance.now()-t0)/1000)*P.speed);
